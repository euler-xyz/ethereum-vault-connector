...
library ExecutionContext {
    ...
    uint internal constant OPERATOR_CALL_LOCK_MASK =
    ...
    function isOperatorCallInProgress(
        EC context
    ) internal pure returns (bool result) {
        result = EC.unwrap(context) & OPERATOR_CALL_LOCK_MASK != 0;
    }

    function setOperatorCallInProgress(
        EC context
    ) internal pure returns (EC result) {
        result = EC.wrap(EC.unwrap(context) | OPERATOR_CALL_LOCK_MASK);
    }

    function clearOperatorCallInProgress(
        EC context
    ) internal pure returns (EC result) {
        result = EC.wrap(EC.unwrap(context) & ~OPERATOR_CALL_LOCK_MASK);
    }
