...
contract EchidnaTest {
    ...
    // call Echidna prepared functions to verify that the cached context value can be reused
    ...
    function impersonateEchidna(
        ...
        hevm.prank(address(vaultEchidna));
        cvc.impersonateEchidna(address(vaultEchidna), onBehalfOfAccount, data);
    }

    function batchEchidna(ICVC.BatchItem[] calldata items) public payable {
        if (items.length > 0) {
            ICVC.BatchItem[] memory _items = new ICVC.BatchItem[](1);
            _items[0].targetContract = address(vaultEchidna);
            _items[0].onBehalfOfAccount = items[0].onBehalfOfAccount;
            ...
            hevm.prank(_items[0].onBehalfOfAccount);
            cvc.batchEchidna(_items);

            try cvc.batchEchidna(items) {} catch {}
        } else {
            cvc.batchEchidna(items);
        }
    }

    function checkAccountStatusEchidna(address account) public payable {
        cvc.checkAccountStatusEchidna(account);
    }

    function checkAccountsStatusEchidna(
        address[] calldata accounts
    ) public payable {
        cvc.checkAccountsStatusEchidna(accounts);
    }

    function requireAccountStatusCheckEchidna(address account) public payable {
        cvc.requireAccountStatusCheckEchidna(account);
    }

    function requireAccountsStatusCheckEchidna(
        address[] calldata accounts
    ) public payable {
        cvc.requireAccountsStatusCheckEchidna(accounts);
    }

    function requireAccountStatusCheckNowEchidna(
        address account
    ) public payable {
        cvc.requireAccountStatusCheckNowEchidna(account);
    }

    function requireAccountsStatusCheckNowEchidna(
        address[] calldata accounts
    ) public payable {
        cvc.requireAccountsStatusCheckNowEchidna(accounts);
    }

    function requireAllAccountsStatusCheckNowEchidna() public payable {
        cvc.requireAllAccountsStatusCheckNowEchidna();
    }

    function forgiveAccountStatusCheckEchidna(address account) public payable {
        cvc.forgiveAccountStatusCheckEchidna(account);
    }

    function forgiveAccountsStatusCheckEchidna(
        address[] calldata accounts
    ) public payable {
        cvc.forgiveAccountsStatusCheckEchidna(accounts);
    }

    function requireVaultStatusCheckEchidna() public payable {
        cvc.requireVaultStatusCheckEchidna();
    }

    function requireVaultStatusCheckNowEchidna(address vault) public payable {
        cvc.requireVaultStatusCheckNowEchidna(vault);
    }

    function requireVaultsStatusCheckNowEchidna(
        address[] calldata vaults
    ) public payable {
        cvc.requireVaultsStatusCheckNowEchidna(vaults);
    }

    function requireAllVaultsStatusCheckNowEchidna() public payable {
        cvc.requireAllVaultsStatusCheckNowEchidna();
    }

    function forgiveVaultStatusCheckEchidna() public payable {
        cvc.forgiveVaultStatusCheckEchidna();
    }

    function requireAccountAndVaultStatusCheckEchidna(
        address account
    ) public payable {
        cvc.requireAccountAndVaultStatusCheckEchidna(account);
    }
}
