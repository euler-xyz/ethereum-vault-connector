...
- Sentinel: A designated EOA or smart contract address authorized by a user to perform a second stage verification of a Permit message. Sentinels act as security gatekeepers, ensuring that operations proceed only if the Permit data is deemed safe. Account Owner MUST explicitly authorize a sentinel address before it can be used for Permit verification. This mechanism significantly enhances security against phishing attacks by requiring an attacker to compromise the sentinel in addition to deceiving the user.
