...
- Checks-deferrable Call: Any function calls that defers the Account and Vault Status Checks, namely: Call, Batch or ControlCollateral call. Checks-deferrable Calls can be nested.
...
1. The Controller Vault MUST NOT be allowed to use a Permit message in order to disable itself for an Account.
...
1. The Controller Vault MUST NOT be allowed to use a Permit message in order to use ControlCollateral.
