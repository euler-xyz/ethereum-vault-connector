...
- Attester: A designated EOA or smart contract address authorized by a user to perform a second stage verification of a Permit message. Attesters act as security gatekeepers, ensuring that operations proceed only if the Permit data is deemed safe. Account Owner MUST explicitly authorize an attester address before it can be used for Permit verification. This mechanism significantly enhances security against phishing attacks by requiring an attacker to compromise the attester in addition to deceiving the user.
