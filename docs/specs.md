...
- Permit Disabled Mode: A security feature that can be activated by the Account Owner. Once activated, it applies to all the Accounts owned by the Account Owner simultaneously, preventing the execution of any permits that were signed by the Account Owner. This mode is particularly useful in emergency situations, such as when a harmful permit message has been signed, to safeguard the owner's assets. The Permit Disabled Mode is a subset of the Lockdown Mode, offering a focused restriction on permit execution.
